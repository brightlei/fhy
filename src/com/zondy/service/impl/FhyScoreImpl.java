...
public class FhyScoreImpl {
	...
	public static int importData(String filepath) throws FileNotFoundException, IOException{
		int successCount = 0;
		...
		if(file.exists()){
			...
			if(dataList != null){
				...
				for(int i=2;i<count;i++){
					...
					fhyJson.put("state", 1);
					fhyJson.put("headimg", "");
					vo = (FhyVo)dao.loadObject("from FhyVo where bh='"+fhyJson.getString("bh")+"'");
					if(vo == null){
						vo = JSON.toJavaObject(fhyJson, FhyVo.class);
					}else{
						voJson = JSONObject.parseObject(JSONObject.toJSONString(vo));
						voJson.putAll(fhyJson);
						voJson.put("xgsj", DateUtils.date2String("yyyy-MM-dd HH:mm:ss"));
...
//				}
	 ...
	 * @return int
	 */
	public static int importQgcjkData(String filepath,String username) throws FileNotFoundException, IOException{
		int successCount = 0;
		File file = new File(filepath);
		if(file.exists()){
			Workbook workbook = ExcelUtils.getWorkbook(filepath);
				...
				for(int i=2;i<count;i++){
					...
					if(!checkRowIsNull(rowList)){
						...
						if(rowList.size()==fieldCount){
							...
							sql = sqlxml.getParamConfig("addPjzbData", sqlParam);
							sqlList.add(sql);
						}
					}
				}
	...
	public static void main(String[] args) {
		...
		filepath = "H:/个人资料/研发产品/04.工务段防护员管理系统/襄阳工务段2018-12-25日计划.xls";
		//filepath = "D:/JavaWorkSpaces/fhy/WebRoot/template/评价指标数据表-Template.xls";
		try {
	...
	public void test() throws Exception{
		...
		int index = 10000;
		String[] gqArr = new String[]{"客整","襄阳线路","西湾","三场线路","小清河","襄阳东","三场维修"};
		...
		for(int i=0;i<2000;i++){
			...
			record.put("zzmm", zzmmArr[RandomUtils.getInt(0, zzmmArr.length-1)]);
			record.put("dqfhzg", "驻站");
			record.put("cjpxsj", DateUtils.date2String("yyyy-MM-dd", DateUtils.calculateByDate(new Date(), -1*RandomUtils.getInt(100, 300))));
			datalist.add(record);
		}
