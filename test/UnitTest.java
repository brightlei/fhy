...
public class UnitTest {
	...
	public void testHibernateTemplate() {
		...
		HibernateTemplate template = (HibernateTemplate) ctx
				.getBean("hibernateTemplate");
		BaseDAOImpl dao = new BaseDAOImpl();
		dao.setMyHibernateTemplate(template);
		List list = dao
				.listAll("select t.*,t1.name as deptname from T_PERSON t,T_DEPT t1 where t.deptid=t1.id and t.deptid='13'");
		System.out.println(list);
		// List list = dao.listAll("select * from T_DIC t");
		// System.out.println(list);
		// String string =
		// "人身安全|防护员岗位|施工管理|干部（工班长）管理|消防安全|交通安全|自轮运转设备管理|探伤作业|道口工管理|巡道（检）人员管理";
		// String[] tArr = string.split("[|]");
		// String sql = "";
		// for(int i=0;i<tArr.length;i++){
		// String code = (i+1)<10?"0"+(i+1):""+(i+1);
		// sql =
		// "insert into T_DIC(dic_name,dic_type,code,text) values('违章类别','wzlb','wzlb-"+code+"','"+tArr[i]+"')";
		...
		// List list =
		// session.createSQLQuery(sql).setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
		// System.out.println(list);
		// sql =
		// "insert into T_DIC(dic_name,dic_type,code,text) values('违章类别','WZLB','WZLB01','AAAAAA')";
	...
	public void testBetchSaveData() {
		...
		for (int i = 0; i < 10; i++) {
			sql = "insert into zd_dic(id,bm,mc,zdlx,flag) values('" + (100 + i)
					+ "','" + i + "','字典-" + i + "','province',0);";
			sb.append(sql);
		}
	...
	public void testHtmlEncode() throws UnsupportedEncodingException {
		...
		System.out.println("反转义HTML:"
				+ StringEscapeUtils.unescapeHtml("<font>chen磊  xing</font>")); // 反转义HTML
		System.out
	...
	public void testImportData() {
		String[] chejian = new String[] { "平林线路车间", "随州线路车间", "枣阳线路车间",
				"襄阳线路车间", "襄阳南线路车间", "胡集线路车间", "襄北线路车间", "陈家湖线路车间", "谷城线路车间",
				"武当山线路车间", "十堰线路车间", "襄阳桥隧车间", "武当山桥隧车间", "云梦线路车间", "轨道车队" };
	...
	public void testImportLogData() {
		...
		int end = 3600 * 24 * 60;
		String sql = "";
		String[] modeArr = new String[] { "用户登录", "防护员管理", "违章违纪", "全过程监控",
				"数据字典", "部门管理", "人员管理", "角色管理", "用户管理" };
		...
		for (int i = 0; i < 1000; i++) {
			...
			String datetime = DateUtils
					.date2String("yyyy-MM-dd HH:mm:ss", date);
			...
			sql = "insert into t_user_oplog(ip,name,username,optype,opcontent,createtime) values('"
					+ ip
					+ "','"
					+ mode
					+ "','admin','"
